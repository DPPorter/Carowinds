...
@Configurable
@TeleOp
public class outtakeBeamTest extends OpMode {
    ...
    public void loop() {
        ...
        boolean ballThere = false;

        int yes = 0;
        for(int i = 10; i > 0; i --)
            if(outtakeBeam.getState()) yes ++;
        if(yes >= 6) ballThere = true;

        if(ballThere) out.reset();
        else there.reset();

        telemetry.addData("outtake", ballThere);
        telemetry.addData("thereTime", there.milliseconds());
        telemetry.addData("outTime", out.milliseconds());


    }

    ElapsedTime there = new ElapsedTime();
    ElapsedTime out = new ElapsedTime();
